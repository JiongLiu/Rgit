...
points(jitter(log(numvisit + 0.1)) ~ jitter(age), data=badhealth,
...
plot(lam_hat[which(badhealth$badh==0)], resid[which(badhealth$badh==0)], 
     xlab="lam_hat", ylab="resid", xlim=c(0,8), ylim=range(resid))
points(lam_hat[which(badhealth$badh==1)], resid[which(badhealth$badh==1)], 
       xlab="lam_hat", ylab="resid", xlim=c(0,8), col="red")
...
mean(y2 > y1)  # the possibility that person 2 would have more doctor visits than person 1
